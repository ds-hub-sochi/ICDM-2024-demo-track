...
class VideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        if args.soft_targets:
            soft_target_mtx = np.load(args.soft_targets)    
            self.label_array = [soft_target_mtx[i] for i in list(cleaned.values[:, 1])]
        else:
    ...
    def _aug_frame(
        ...
        buffer = spatial_sampling(
            buffer,
            spatial_idx=self.args.spatial_idx,
            min_scale=256,
            max_scale=320,
    ...
    def loadvideo_decord(self, sample, sample_rate_scale=1):
        ...
        if self.mode == 'test':
            ...
            if args.sampling_type == 'last_frame':
                while len(all_index) < self.clip_len:
                    all_index.append(all_index[-1])
            if args.sampling_type == 'circle':
                if len(all_index) < self.clip_len:
                    for i in range(int(self.clip_len/len(all_index))):
                        all_index += all_index
                    all_index = all_index[0 : self.clip_len]
            vr.seek(0)
        ...
        for i in range(self.num_segment):
            if seg_len <= converted_len:
                if args.sampling_type == 'last':
                    index = np.linspace(0, seg_len, num=seg_len // self.frame_sample_rate)
                    index = np.concatenate((index, np.ones(self.clip_len - seg_len // self.frame_sample_rate) * seg_len))
                    index = np.clip(index, 0, seg_len - 1).astype(np.int64)
                if args.sampling_type == 'circle':
                    index = [x for x in range(0, len(vr), self.frame_sample_rate)]
                    for i in range(int(self.clip_len/len(index))):
                        index += index
                    index = index[0:self.clip_len]
            else:
...
class GSLVideoClsDataset(Dataset):
    """Load your own video classification dataset."""
